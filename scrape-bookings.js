...
const OUT_FILE = path.resolve(__dirname, '..', 'HOV-Daily-Checklist', 'src', 'data', 'bookings.json');
...
function writeOutput(bookings){
  const data = {
    bookings: bookings,
    delivery: bookings.filter(b => /udlever|pickup|delivery/i.test(b.status || b.category || '')).length,
